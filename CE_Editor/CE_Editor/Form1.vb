...
Public Class Form1
    ...
    Private Sub SaveToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles SaveToolStripMenuItem.Click
        ...
        If Not Saved_File = "" Then
            ...
            current_part = CInt(get_number_value_in_string(DataGridView1(0, 0).Value.ToString(), 1))

            part_array = New Byte(0) {}
            body_part_count = 0
            'tekken57 new code
            'current_mask = CInt(DataGridView1(0, 0).Value.ToString().Substring(9, 1))
            current_mask = CInt(get_number_value_in_string(DataGridView1(0, 0).Value.ToString(), 1))
            ...
            Do While current_row < DataGridView1.RowCount
                ...
                Else
                    ...
                    Else
                        ...
                        If Not current_part = CInt(get_number_value_in_string(DataGridView1(0, current_row).Value.ToString(), 1)) Then
                            'putting the number arrays into a mask container
                            make_mask()
                            'Make a new part together and adding it to any other parts
                            make_body_part()
                        Else
                            ...
                            If Not current_mask = CInt(get_number_value_in_string(DataGridView1(0, current_row).Value.ToString(), 2)) Then
                                'putting the number arrays into a mask container
                                make_mask()
                            Else
    ...
    Function get_number_value_in_string(strValue As String, instance As Int32)
        Dim underscoreLocations(3) As Int32
        Dim rowName As String = strValue

        Dim regex As New System.Text.RegularExpressions.Regex("_")
        Dim i As Int32 = 0
        For Each match As RegularExpressions.Match In regex.Matches(rowName)
            underscoreLocations(i) = match.Index
            i = i + 1
        Next
        ...
        If instance = 2 Then
            currentObjectName = rowName.Substring(underscoreLocations(instance) + 1, strValue.Length - (underscoreLocations(instance) + 1))
        Else
            currentObjectName = rowName.Substring(underscoreLocations(instance) + 1, (underscoreLocations(instance + 1) - (underscoreLocations(instance) + 1)))
    ...
    Private Sub make_mask()
        ...
        If current_row < DataGridView1.RowCount Then
            ' new code by tekken57 fixing errors with two digit numbers
            'current_mask = CInt(DataGridView1(0, current_row).Value.ToString().Substring(9, 1)) --- old pozzum code

            '----moved to function
            'Dim underscoreLocations(3) As Int32
            'Dim rowName As String = DataGridView1(0, current_row).Value.ToString()
            ...
            'Dim currentObjectName As String = DataGridView1(0, current_row).Value.ToString().Substring(underscoreLocations(2) + 1, DataGridView1(0, current_row).Value.ToString().Length - (underscoreLocations(2) + 1))
            '----
            Dim rowName As String = DataGridView1(0, current_row).Value.ToString()
            Dim currentObjectName As String = get_number_value_in_string(rowName, 2)



            current_mask = CInt(currentObjectName.Trim())
    ...
    Sub make_body_part()
        ...
        If current_row < DataGridView1.RowCount Then
            ...
            'tekken57 new code
            current_part = CInt(get_number_value_in_string(DataGridView1(0, current_row).Value.ToString(), 1))
    ...
    Private Sub open_faces(ByVal Source As String)
        ...
        For i As Integer = 0 To New_Faces.Count - 1 Step 2
            ...
            'new code by tekken57 to fix error where numbers are 2 digits
            'Dim Temp_Object_Number As Integer = New_Faces(i).Substring(6, 1) ----- pozzum's original code
            Dim Temp_Object_Number As Integer = Convert.ToInt32(New_Faces(i).ToLower.Replace("object", ""))
